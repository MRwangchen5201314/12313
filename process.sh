...
readonly LOG_RETENTION_DAYS=9           # 日志保留天数：9天（与文件删除时间一致）
...
cleanup_old_logs() {
    ...
    }
    {
        ...
        if (NF >= 6 && $1 ~ /^[0-9]{4}$/ && $2 ~ /^[0-9]{2}$/ && $3 ~ /^[0-9]{2}$/) {
...
cleanup() {
    local exit_code=$?
    # 安全检查：确保 WORK_DIR 不为空且不是根目录或系统关键目录
    if [[ -n "$WORK_DIR" && "$WORK_DIR" != "/" && "$WORK_DIR" != "/tmp" && "$WORK_DIR" != "$HOME" ]] && [[ -d "$WORK_DIR" ]]; then
        # 额外检查：确保目录在预期位置（包含 .tmp-work-）
        if [[ "$WORK_DIR" == *".tmp-work-"* ]]; then
...
if [[ $source_count -gt 0 ]]; then
    ...
    while IFS= read -r url; do
        # 重置循环变量，防止污染（按类别分组）
        # 文件相关变量
        unset cache_file temp_file file_head
        # 时间相关变量
        unset file_mtime cache_age download_time
        # 下载控制变量
        unset download_success retry backoff
        # 统计和错误变量
        unset downloaded_size rules_count error_msg
        
        [[ -z "$url" ]] && continue
        ((current++))
        
        # 显示当前处理的URL（截取前60字符）
        short_url=$(printf '%s' "$url" | cut -c1-60)
        ...
        # 验证 URL 内容（检查非法字符）
        if [[ "$url" =~ [$'\n\r\t\0'] ]]; then
            echo "    └─ ❌ URL包含非法字符（换行符、制表符等）" >&2
            ((failed_count++))
            continue
        fi
        
        cache_file="$CACHE_DIR/$(printf '%s' "$url" | md5sum | cut -d' ' -f1)"
        temp_file=$(mktemp "$WORK_DIR/download.XXXXXX" 2>/dev/null)
        ...
        curl_output=$(mktemp "$WORK_DIR/curl.XXXXXX" 2>/dev/null)
        if [[ -z "$curl_output" || ! -f "$curl_output" ]]; then
            echo "    └─ ❌ 无法创建curl输出临时文件" >&2
        ...
        if [[ $download_success -eq 1 ]]; then
            ...
            if mv "$temp_file" "$cache_file" 2>/dev/null; then
                if cat "$cache_file" >> "$rules_file" 2>/dev/null; then
                    ...
                    # 清理旧日志，保留最近10天的所有记录
                    [[ -n "$PERF_LOG" && -f "$PERF_LOG" ]] && cleanup_old_logs "$PERF_LOG" "$LOG_RETENTION_DAYS" || true
            ...
            mkdir -p "$LOG_DIR" 2>/dev/null || true
            # 清理旧日志，保留最近10天的所有记录
            [[ -n "$FAIL_LOG" && -f "$FAIL_LOG" ]] && cleanup_old_logs "$FAIL_LOG" "$LOG_RETENTION_DAYS" || true
            echo "$(date '+%Y-%m-%d %H:%M:%S') | $url | ${error_msg:-未知错误}" >> "$FAIL_LOG" 2>/dev/null || true
        fi
    done <<< "$source_list"
    set -e  # 恢复错误退出
...
if [[ -f "README.template.md" ]]; then
    ...
    if ! cp "README.template.md" "$README_FILE" 2>/dev/null; then
        echo "  ⚠️  无法复制模板文件，将使用默认生成" >&2
    fi
    # 检查是否有占位符，如果有则替换
...
# 日志文件自动清理：删除30天前的日志文件（防止长期不运行导致日志堆积）
echo "  └─ 清理旧日志文件..."
if [[ -d "$LOG_DIR" ]]; then
    # 删除30天前的日志文件
    deleted_count=$(find "$LOG_DIR" -name "*.log" -mtime +30 -delete 2>/dev/null | wc -l || echo 0)
    if [[ $deleted_count -gt 0 ]]; then
        echo "  └─ 删除旧日志文件：$deleted_count 个"
    fi
    
    # 检查日志目录大小，如果超过100MB给出警告
    log_size_mb=$(du -s "$LOG_DIR" 2>/dev/null | awk '{printf "%.0f", $1/1024}' || echo 0)
    if [[ $log_size_mb -gt 100 ]]; then
        echo "  ⚠️  警告：日志目录过大（${log_size_mb}MB），建议手动清理" >&2
        echo "     日志目录：$LOG_DIR" >&2
    fi
fi
